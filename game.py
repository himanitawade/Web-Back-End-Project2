...
async def create_game(data):
    ...
    if data:
        ...
        while await db.fetch_one(
            ...
            values={"username": data[username], "answerid": word[0]},
        ):
            word = await db.fetch_one(
                "SELECT answerid FROM answer ORDER BY RANDOM() LIMIT 1"
            )
        ...
        values = {"username": data[username], "answerid": word[0], "gameid": cur}
        cur = await db.execute(query=query, values=values)
...
async def all_games():
    ...
    if data:
        ...
        games_val = await db.fetch_all( "SELECT * FROM game as a where gameid IN (select gameid from games where username = :username) and a.gstate = :gstate;", values = {"username":data[username],"gstate":"In-progress"})
