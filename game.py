...
async def create_game():
    ...
    if auth and auth.username and auth.password:
        ...
        values = {"username": auth.username, "answerid": word[0], "gameid": cur}
        cur = await db.execute(query=query, values=values)
...
@app.route("/onegame/<int:gameid>", methods=["GET"])#?id=guess --To Do function chge this to get the values in the search param format like books search api given by prof
